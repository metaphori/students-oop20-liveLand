...
public class SimulationPanel extends JPanel {
    ...
    public void updateSimulation() {
        ...
        for(PersonTicket p: people.keySet()) {
            if(p.getAge()>12) {
                adult.add(DesignPerson.createAdult(people.get(p).getFirst(), 
                        people.get(p).getSecond(), ADULT_RADIUS));
                System.out.println("Posizione di " + p.toString() + "= x: " + map.get(p).getFirst() + "y: " + map.get(p).getSecond() );
            } else {
                baby.add(DesignPerson.createBaby(people.get(p).getFirst(), 
                        people.get(p).getSecond(), BABY_RADIUS));  
            }
            this.adult = adult;
            this.baby = baby;
            this.repaint();
